...
export default function ProductDetail() {
  ...
  const { data: reviewsData, refetch: refetchReviews } = useQuery({
    queryKey: ['product', { product: product?.id }],
    queryFn: () => ratingApi.get({ product: product?.id }),
    // staleTime: 3 * 60 * 1000,
  ...
  const nameAgency = product?.agency.name
  const queryConfig = { agency: nameAgency, page: queryParamsDefault.page, page_size: 12 }
  ...
  const updateReply = useMutation({
    mutationFn: (data: any) => ratingApi.update(data),
    onSuccess: () => {
      refetchReviews()
    }
  })
  ...
  const activeListImg = useMemo(
    () => (product ? product.image_list.slice(...indexImg) : []),
    [product, indexImg]
  )
  ...
  useEffect(() => {
    if (product && product.image_list.length > 0) {
      setActiveImg(product.image_list[0].image)
      setSize(product.item[0])
    }
  }, [product])

  const handleNext = () => {
    if (product && indexImg[1] < product.image_list.length) {
  ...
  const handlePrevModal = () => {
    if (product) {
      let indexImgCurrent = 0
      for (let i = 0; i < product.image_list.length; i++) {
        if (product.image_list[i].image === activeImg) {
          indexImgCurrent = i
          break
        }
      }
      if (indexImgCurrent <= 0) {
        const lastIndexImg = product.image_list.length - 1
        return setActiveImg(product.image_list[lastIndexImg].image)
      }
      return setActiveImg(product.image_list[indexImgCurrent - 1].image)
    }
  }
  ...
  const handleNextModal = () => {
    if (product) {
      ...
      if (indexImgCurrent === product.image_list.length - 1) {
        return setActiveImg(product.image_list[0].image)
      }
      return setActiveImg(product.image_list[indexImgCurrent + 1].image)
    }
  }
  ...
  const handleBuyNow = () => {
    if (isAuthenticated) {
      buyPurchaseMutation.mutate({
        ...
        total_price: product.price * buyCount,
        status: 'creating'
      })
    } else {
      navigate(path.login)
  ...
  const updateReview = (id: string, reply: string) => {
    setReviews((prevReview) => {
      const newReviews = prevReview.map((review) => {
        if (review.id === id) {
          return { ...review, reply: reply } // Cập nhật rating mới
        }
        return review
      })
        ...
        <div className='border-b-4 border-b-orange bg-[#f5f5f5] pb-[60px] pt-10'>
          ...
          <div className='container flex flex-col rounded-[3px] bg-white shadow md:flex-row'>
                  ...
                  >
                    ...
                    <div className='flex w-full flex-1 flex-col pb-[30px] pl-3 pt-10'>
                      ...
                      <div className='mt-5 flex flex-1 flex-wrap content-start justify-start overflow-auto'>
                        {product.image_list.map((img: any, index: number) => {
                          const isActive = img.image === activeImg
                          return (
            ...
            <div className='p-[15px]'>
              <div
                className='relative w-full cursor-zoom-in overflow-hidden h-[450px] w-[450px] '
                role='button'
                aria-hidden
                tabIndex={0}
                onClick={() => setOpen((prev) => !prev)}
              ...
              >
                <img
                  className=' pointer-events-none absolute left-0 top-0 h-fullobject-cover h-[450px] w-[450px]'
                  src={activeImg}
                  alt={product?.name}
                  ref={imgRef}
                />
              ...
              <div className='relative mt-[10px] flex items-center gap-[10px]'>
                {activeListImg.map((img: any, index: number) => {
                  const isActive = activeImg === img.image
                  return (
                    <div
                      className='relative cursor-pointer'
                      key={index}
                      onMouseEnter={() => setActiveImg(img.image)}
                      onClick={() => setOpen((prev) => !prev)}
                      aria-hidden
                    >
                      <img
                ...
                <button
                  ...
                  className={classNames(
                    ...
                    {
                      'cursor-default': indexImg[1] === product.image_list.length
                    }
                  )}
                >
                  <svg
            ...
            <div className='flex-1 px-2 py-[20px]'>
              ...
              <div className='mt-4 flex flex-wrap items-center gap-y-3'>
                ...
                <div className='flex items-center gap-[5px] px-[15px]'>
                  <span className='text-base uppercase'>{formatSocialNumber(product.buyed_total)}</span>
              ...
              <div className='mt-4 bg-[#fafafa] px-[20px] py-[15px]'>
                <div className='flex flex-wrap items-center gap-[10px]'>
                  <div className='text-base text-gray-400 line-through'>
                    ₫{formatPriceNumber(product.old_price)}
                  </div>
                  ...
                  <div className='ml-[5px] rounded-sm bg-orange px-1 py-[0.5px] text-xs font-semibold uppercase text-white'>
                    {Math.round(((product.old_price - product.price) / product.old_price) * 100)}% {t('off')}
                  </div>
                </div>
              ...
              <div className='mt-8 hidden border-t border-t-gray-200 lg:block'>
                <div className='flex items-center gap-[20px] px-[20px] py-9'>
                  <div className='flex items-center gap-[5px]'>
                    <img className='h-[20px]' src='src/assets/images/security.png' alt='logo' />
          ...
          <div className='container mt-[15px] bg-white shadow'>
            <div className='bg-[#fafafa] p-[14px] text-lg capitalize'>{t('product rating')}</div>
            <div className='mt-15 hidden border-t border-t-gray-200 lg:block'>
              {reviews?.map((review: any) => (
                <div className=' px-[20px] py-3 '>
                  <div className='container flex flex-col rounded-[3px] bg-white shadow md:flex-row border border-orange'>
                    <div className='flex-1 px-2 py-[20px]'>
                      <div className='flex cursor-pointer items-center gap-1 py-4 text-[15px] font-medium'>
                        <img
                          className='h-[20px] w-[20px] rounded-full object-cover'
                          alt='avatar'
                          src={review.product.avatar}
                        />
                        <span>{review.product.name}</span>
                      </div>
                      <div className=' flex flex-wrap items-center gap-y-3'>
                        <div className='flex items-center gap-[5px] border-r border-r-gray-300 pr-[15px]'>
                          <RatingStar rating={review.rate_point} size={13} />
                        </div>
                        <div className='flex items-center gap-[5px] border-r border-r-gray-300 px-[10px]'>
                          <div className='text-[15px] font-small'>
                            {reformatDateString(review.created_at)}
                          </div>
                        </div>
                        <div className='flex items-center gap-[5px] px-[10px]'>
                          <div className='text-[15px] font-small'>
                            Phân loại hàng: {review.product.product_name}, {review.product.size}
                          </div>
                        </div>
                      </div>
                  ...
                  {(review.replied || (profile && profile.is_superuser)) && (
                    <div
                      className='container flex flex-col rounded-[3px] shadow md:flex-row'
                      style={{ backgroundColor: '#c4c4c7' }}
                    >
                      <div className='flex-1 px-2 py-[7px]'>
                        <div className='flex cursor-pointer items-center gap-1 py-4 text-[15px] font-medium'>
                          <span>Phản hồi của người bán |</span>
                          <div className='flex items-center gap-[5px] border-r border-r-gray-300 px-[10px]'>
                            <div className='text-[15px] font-small'>
                              {reformatDateString(review.updated_at)}
                            </div>
                          </div>
                        </div>
                        {review.replied && (
                          <div className='text-[15px] font-medium'>
                            <div
                              className='m-[15px] mt-[3px] whitespace-pre-wrap text-sm leading-loose'
                              dangerouslySetInnerHTML={{
                                __html: DOMPurify.sanitize(`${review.reply}`)
                              }}
                            />
                          </div>
                        )}
                        {profile && profile.is_superuser && review.replied === false && (
                          <div
                            className='grid grid-cols-12 gap-4 px-5 py-[16px]'
